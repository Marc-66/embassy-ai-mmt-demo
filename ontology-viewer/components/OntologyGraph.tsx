...
export default function OntologyGraph({ data, onNodeSelect }: OntologyGraphProps) {
  ...
  useEffect(() => {
    ...
    svg.append('defs').selectAll('marker')
      .data(['subClassOf', 'objectProperty', 'dataProperty', 'instanceOf'])
      .enter().append('marker')
      .attr('id', d => `arrow-${d}`)
      ...
      .attr('fill', d => {
        switch (d) {
          case 'subClassOf': return '#94a3b8';
          case 'objectProperty': return '#3b82f6';
          case 'dataProperty': return '#f59e0b';
          case 'instanceOf': return '#ec4899';
    ...
    const circles = node.append('circle')
      ...
      .on('click', (event, d) => {
        ...
        if (highlightNeighbors) {
          ...
          link
            .style('opacity', (e: OntologyEdge) => isEdgeConnected(e, d.id) ? 1 : 0.1)
            .attr('stroke-width', (e: OntologyEdge) => isEdgeConnected(e, d.id) ? 3 : 2);
          
          linkLabel
            .style('opacity', (e: OntologyEdge) => isEdgeConnected(e, d.id) ? 1 : 0.1);
          
          nodeLabels
            .style('opacity', (n: OntologyNode) => connectedNodes.has(n.id) ? 1 : 0.3);
          
          typeBadges
            .style('opacity', (n: OntologyNode) => connectedNodes.has(n.id) ? 1 : 0.3);
        } else {
          circles
            .attr('stroke', (n: OntologyNode) => n.id === d.id ? '#000' : '#fff')
            .attr('stroke-width', (n: OntologyNode) => n.id === d.id ? 3 : 2);
        }
      })
      .on('dblclick', (event, d) => {
        circles
    ...
    const nodeLabels = node.append('text')
      ...
      .attr('fill', '#e5e7eb')
      .text((d: OntologyNode) => getLocalizedText(d.label, language))
  ...
  }, [data, onNodeSelect, highlightNeighbors, language]);
      ...
      <div className="absolute top-4 right-4 bg-gray-800 border border-gray-600 p-3 rounded shadow-md text-sm">
        ...
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-gray-300">Object / URI</span>
          </div>
          <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-gray-300">Class membership</span>
          </div>
          <div className="flex items-center gap-2">
